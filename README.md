...
3. **Environment variables:** copy the template and set secrets.
   ...
   - `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY` — Supabase project credentials (service role needed for storage writes).
   - `SUPABASE_STORAGE_BUCKET` — bucket name for stored PDFs/materials.
   - `SUPABASE_AVATAR_BUCKET` — dedicated bucket for profile photos (allow PNG/JPEG in its MIME policy).
